...
return {
	...
	initUI = function(self)
		...
		noteSplashLeft.x = 255 - 1025
		noteSplashRight.x = 780 - 1025
		noteSplashUp.x = 600 - 1025
		noteSplashDown.x = 425 - 1025
	...
	update = function(self, dt)
		...
		enemy:update(dt)
		boyfriend:update(dt)
		noteSplashLeft:update(dt)
		noteSplashRight:update(dt)
		noteSplashUp:update(dt)
		noteSplashDown:update(dt)
	...
	updateUI = function(self, dt)
		...
		for i = 1, 4 do
			...
			if #boyfriendNote > 0 then
				if (not settings.downscroll and boyfriendNote[1].y - musicPos <= -400) or (settings.downscroll and boyfriendNote[1].y - musicPos >= 400) then
					voices:setVolume(1)
					...
					if boyfriendNote[1]:getAnimName() == "hold" or boyfriendNote[1]:getAnimName() == "end" then
						if useAltAnims then
							if (not boyfriend:isAnimated()) or boyfriend:getAnimName() == "idle" then self:safeAnimate(boyfriend, curAnim .. " alt", true, 2) end
						else
							if (not boyfriend:isAnimated()) or boyfriend:getAnimName() == "idle" then self:safeAnimate(boyfriend, curAnim, true, 2) end
						end
					else
						if useAltAnims then
							self:safeAnimate(boyfriend, curAnim .. " alt", false, 2)
						else
			...
			if notMissed[noteNum] and #enemyNote > 0 and input:down(curInput) and ((not settings.downscroll and enemyNote[1].y - musicPos <= -400) or (settings.downscroll and enemyNote[1].y - musicPos >= 400)) and (enemyNote[1]:getAnimName() == "hold" or enemyNote[1]:getAnimName() == "end") then
				voices:setVolume(1)
		...
		elseif health > 20 and boyfriendIcon:getAnimName() == "pico losing" then
			boyfriendIcon:animate("pico", false)
		elseif health <= 20 and boyfriendIcon:getAnimName() == "pico" then
	...
	drawUI = function(self)
		love.graphics.push()
			...
			for i = 1, 4 do
				...
				enemyArrows[i]:draw()
				if boyfriendArrows[i]:getAnimName() == "off" then
